...
public class VitaLoader extends GhidraScript {
    ...
    Consumer<String> errorConsumer = a -> System.out.println(a);
		ByteArrayProvider elfHeaderProvider = new ByteArrayProvider(elfHeaderBytes);
		return new ElfHeader(elfHeaderProvider, errorConsumer);
	}
	...
	private void processExports(Program program, NidDatabase db, Memory memory, MemoryBlock block, String moduleName, Address exportsAddress, SceModuleExports exports) throws Exception {
		println("Exports NID: " + String.format("0x%08X", exports.library_nid) + ' ' + exports.library_name);
    ...
    String libraryName = "main";
    if (exports.library_name != 0) {
      ...
      libraryName = libraryNameBinaryReader.readNextAsciiString();
    }
	...
	public void run() throws Exception {
		...
		String lastDir = Preferences.getProperty(Preferences.PROJECT_DIRECTORY);
		if (lastDir != null)
			fileChooser.setCurrentDirectory(new File(lastDir));
		fileChooser.setTitle("Choose NID database directory");
		...
		if (selectedDir == null) {
			Msg.showError(this, null, "VitaLoader", "A NID database directory is needed!");
			return;
		}
		if (!selectedDir.isDirectory()) {
			Msg.showError(this, null, "VitaLoader", "A directory must be selected.");
			return;
    }
		
    File[] dbFiles = selectedDir.listFiles((dir, name) -> name.toLowerCase().endsWith(".yml") || name.toLowerCase().endsWith(".yaml"));

		if (dbFiles == null || dbFiles.length == 0) {
			Msg.showError(this, null, "VitaLoader", "No NID database files (.yml, .yaml) found in the selected directory.");
			return;
		}
        ...
        println("Reading NID DB: " + dbFile.getName() + "...");
